 ...
 */
#if ENABLED(THERMAL_PROTECTION_HOTENDS) && NEED_HEAT_HEAD
//YSZ-COMMENT:这里表示,当温度与目标温度之差的绝对值大于THERMAL_PROTECTION_HYSTERESIS,一直保持超过THERMAL_PROTECTION_PERIOD将认为温度恢复不了,报错
 ...
 */
#if ENABLED(THERMAL_PROTECTION_BED) && NEED_HEAT_BED
...
#if ENABLED(FIX_MOUNTED_PROBE) && DISABLED(Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN) //使用限位找Z极限点,使用探头找Z的0点
